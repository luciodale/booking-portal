    ...
    <div className="bg-secondary/30 rounded-xl p-5 border border-border">
      ...
      <div className="grid grid-cols-7 gap-1">
        {monthDays.map((day, cellIndex) => {
          ...
          if (!day) {
            return <div key={cellKey} className="h-14" />;
          }

          const state = getDayState(day);
          // Use base class but exclude isPriced coloring
          const baseClass = getDayClassName({ ...state, isPriced: false });
          ...
          return (
            <button
              ...
              className={cn(
                "relative h-14 flex flex-col items-center justify-start pt-1 rounded-md text-xs font-medium",
                baseClass
              )}
            >
